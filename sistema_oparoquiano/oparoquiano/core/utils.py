...
def paginacao(request, objeto, number_pages=20):
    page = request.GET.get('page', 1)
    paginator = Paginator(objeto, number_pages)

    try:
        objeto = paginator.page(page)
    except PageNotAnInteger:
        objeto = paginator.page(1)
    except EmptyPage:
        objeto = paginator.page(paginator.num_pages)
